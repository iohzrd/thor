...
// TODO import java.security.interfaces.XECPublicKey;
...
public class KeyShareExtension extends Extension {
    ...
    static {
        CURVE_KEY_LENGTHS = new HashMap<>();
        CURVE_KEY_LENGTHS.put(secp256r1, 65);
        CURVE_KEY_LENGTHS.put(x25519, 32);
        CURVE_KEY_LENGTHS.put(x448, 56);
    }
    public static final List<TlsConstants.NamedGroup> supportedCurves = Arrays.asList(secp256r1, x25519, x448);
    ...
    protected int parseKeyShareEntry(ByteBuffer buffer, boolean namedGroupOnly) throws TlsProtocolException {
        ...
        if (namedGroupOnly) {
            ...
            /*else if (namedGroup == x25519 || namedGroup == x448) {
                ...
                LogUtils.error(getClass().getSimpleName(), "TODO " + namedGroup);
            }
        }
    ...
    public byte[] getBytes() {
        ...
        for (KeyShareEntry keyShare: keyShareEntries) {
            ...
            /* TODO else if (keyShare.getNamedGroup() == x25519 || keyShare.getNamedGroup() == x448) {
