    ...
    void parsePackets(int datagram, Instant timeReceived, ByteBuffer data) {
        try {
            while (data.remaining() > 0) {
                LogUtils.error(getClass().getSimpleName(), "Parsed packet with size " + data.position() + "; " + data.remaining() + " bytes left.");
                ...
                try {
                    ...
                    LogUtils.error(getClass().getSimpleName(), "Discarding packet (" + nrOfPacketBytes + " bytes) that cannot be decrypted (" + cannotParse + ")");
                    ...
                    LogUtils.error(getClass().getSimpleName(), "Dropping invalid packet");
                }
            ...
            LogUtils.error(getClass().getSimpleName(), "Processed all packets ");
            ...
            LogUtils.error(getClass().getSimpleName(), throwable);
        }
    }
