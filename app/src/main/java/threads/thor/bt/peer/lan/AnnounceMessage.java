...
import static threads.thor.bt.net.InternetProtocolUtils.getLiteralIP;
...
class AnnounceMessage {
    ...
    static int calculateMessageSize(int numberOfTorrentIds) {
    ...
    static AnnounceMessage readFrom(ByteBuffer buffer) {
    ...
    private static AnnounceMessage parse(byte[] bytes) {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            String value = token.substring(k + keyValueSeparator.length());
            switch (key) {
                case "Host": {
                    // ignore
                    break;
